    ...
    <div className="min-h-screen bg-[#F0F4F8] pb-24 relative overflow-hidden">
      ...
      <div className="pt-8 px-6 relative z-10">
        <div className="flex justify-between items-center mb-6">
          <div>
            <p className="text-gray-500 text-sm font-medium mb-0.5">{t('welcome')},</p>
            <h2 className="text-3xl font-black text-gray-800 tracking-tight">
              {user.displayName || user.name || t('student')} <span className="inline-block animate-wave">👋</span>
            </h2>
          </div>
          ...
          <div className="bg-white/80 backdrop-blur-sm p-2 pl-3 pr-4 rounded-2xl shadow-sm border border-white/50 flex items-center space-x-2 transition-transform hover:scale-105">
             ...
             <div className="flex flex-col leading-none">
                ...
                <span className="text-[10px] text-gray-500 font-bold uppercase">{t('xp')}</span>
             </div>
          </div>
        </div>

        {/* KUNLIK VAZIFA BANNERI */}
          ...
          <div className="relative z-10">
            <div className="flex items-center mb-3">
                <span className="bg-white/20 px-2 py-1 rounded-lg text-xs font-bold backdrop-blur-md border border-white/10 flex items-center">
                    <Star className="w-3 h-3 mr-1 text-yellow-300 fill-yellow-300" /> {t('daily_task')}
                </span>
            </div>
            <h3 className="font-bold text-xl mb-1 w-2/3 leading-tight">{t('daily_task_title')}</h3>
            <p className="text-blue-100 text-sm mb-4 opacity-90">{t('daily_task_desc')}</p>
            ...
            <button className="bg-white text-blue-600 px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-blue-50 transition flex items-center">
              <Play className="w-4 h-4 mr-2 fill-current" /> {t('start')}
            </button>
          </div>
        ...
        <div className="flex items-center justify-between mb-4">
            <h3 className="font-bold text-xl text-gray-800">{t('my_courses')}</h3>
            <span className="text-blue-600 text-sm font-bold bg-blue-50 px-3 py-1 rounded-full">{t('all')}</span>
        </div>
              ...
              return (
                  ...
                  >
                      ...
                      <div className="relative z-10 flex-1">
                          {/* KURS NOMINI TARJIMA QILAMIZ: t('chemistry'), t('physics') */}
                          <h4 className="font-black text-lg text-gray-800 mb-1">{t(course.id)}</h4>
                          <div className="flex items-center text-gray-500 text-xs font-medium mb-3">
                              <span className="bg-white px-2 py-0.5 rounded-md shadow-sm border border-gray-100 mr-2">
                                {course.lessons.length} {t('lesson_count')}
                              </span>
                              <TrendingUp className="w-3 h-3 mr-1" /> 25% {t('completed')}
                          </div>
