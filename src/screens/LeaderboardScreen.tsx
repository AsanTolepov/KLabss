...
// QO'SHILDI: doc, getDoc, where funksiyalari kerak bo'ladi
...
const LeaderboardScreen = () => {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        ...
        // 1. TOP 50 talikni olamiz (foydalanuvchi so'ragandek)
        ...
        if (currentUserId) {
            // Ro'yxat ichidan qidiramiz
            const myIndex = usersList.findIndex(u => u.uid === currentUserId);
            
            if (myIndex !== -1) {
                // Agar TOP 50 ichida bo'lsa
                setCurrentUserRank(myIndex + 1);
                setCurrentUserData(usersList[myIndex]);
            } else {
                // 2. MUHIM QISM: Agar foydalanuvchi TOP 50 da bo'lmasa (masalan 56-o'rin)
                
                // A) Foydalanuvchi ma'lumotini alohida olamiz
                ...
                if (userDocSnap.exists()) {
                    ...
                    // B) Reytingni hisoblash: XP si biznikidan baland bo'lgan barcha userlarni sanaymiz
                    ...
                    // Rank = mendan tepada turganlar soni + 1
                    const realRank = higherRankSnap.size + 1;
                    setCurrentUserRank(realRank);
                }
            }
        }
      } catch (error) {
  ...
  // Ro'yxatda qolganlarni ko'rsatamiz (4 dan 50 gacha)
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-36">
                        ...
                        <>
                            ...
                            <div className="mt-5 text-center w-full">
                                <p className="font-bold text-[11px] leading-tight text-gray-700 line-clamp-2 h-8 flex items-center justify-center">
                                    {top2.displayName || top2.name || "O'quvchi"}
                                </p>
                                <p className="text-[10px] text-blue-600 font-bold">{top2.xp || 0} XP</p>
                            </div>
                        ...
                        <>
                            ...
                            <div className="mt-5 text-center w-full">
                                <p className="font-bold text-xs leading-tight text-gray-800 line-clamp-2 h-8 flex items-center justify-center">
                                    {top1.displayName || top1.name || "Chempion"}
                                </p>
                                <p className="text-xs text-blue-600 font-black">{top1.xp || 0} XP</p>
                            </div>
                ...
                <div className="flex flex-col items-center w-[30%]">
                    {top3 && (
                        <>
                            ...
                            <div className="mt-5 text-center w-full">
                                <p className="font-bold text-[11px] leading-tight text-gray-700 line-clamp-2 h-8 flex items-center justify-center">
                                    {top3.displayName || top3.name || "O'quvchi"}
                                </p>
                                <p className="text-[10px] text-blue-600 font-bold">{top3.xp || 0} XP</p>
                            </div>
                    ...
                    <div key={index} className={`flex items-center p-3 rounded-xl border transition-colors ${u.uid === currentUserId ? 'bg-blue-50 border-blue-300 shadow-sm' : 'bg-white border-gray-100'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                            <p className={`font-bold text-sm truncate ${u.uid === currentUserId ? 'text-blue-700' : 'text-gray-800'}`}>
                                {u.displayName || u.name || "Foydalanuvchi"}
                            </p>
                        ...
                        <div className="text-right pl-2">
                            ...
                            <span className="text-[9px] text-gray-400 font-bold uppercase">XP</span>
                        </div>
                    </div>
                ))}
            </div>
        ...
        {/* Agar user ma'lumoti bor bo'lsa va u 3-o'rindan pastda bo'lsa (hatto ro'yxatda ko'rinmasa ham) */}
        {currentUserData && currentUserRank && currentUserRank > 3 && (
            <div className="fixed bottom-[70px] left-0 right-0 mx-auto w-[92%] max-w-md z-30 animate-in slide-in-from-bottom-10 fade-in duration-500">
                <div className="bg-slate-800 text-white p-3 rounded-xl shadow-2xl flex justify-between items-center border border-slate-600 backdrop-blur-md bg-opacity-95">
                    ...
                    <div className="text-lg font-black text-blue-400 mr-2">{currentUserData.xp || 0} XP</div>
                </div>
            </div>
        )}
    </div>
